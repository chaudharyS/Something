...
var buy = false, id, message="", currentUser = "";
...
app.get("/user_home", isLoggedIn, function(req, res) {
    Buy.find({username : currentUser}, function(err, foundBuy) {
        if(err){
            ...
            res.render("user_home",{user : foundBuy});
        }
    });
           ...
           Buy.find({username : ""+currentUser}, function(err, foundBuy) {
               if(err){
                   ...
                   if(foundBuy.length==0){
                       Buy.create({username : ""+currentUser, buys : [{policy : foundInsurance.title, time : 1, sum : 100000}]}, function(err, creBuy){
                          if(err){
                              console.log(err);
                          } 
                          else{
                   ...
                   var new_buy = {policy : foundInsurance.title, time : 1, sum : 100000};
